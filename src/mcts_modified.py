...
def traverse_nodes(node, board, state, identity):
    ...
    if node.untried_actions:
        ...
        curr_p = board.current_player(state)
        best_move = UCT(node, 0 if curr_p == identity else 1)
        state = board.next_state(state, best_move.parent_action)
        return traverse_nodes(best_move, board, state, identity)
